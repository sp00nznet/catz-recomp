...
Run: python tools/gen_image.py game/CATZ/CATZ.EXE
"""
import sys
import os
import struct
...
def main():
    ...
    hdr.append('#ifndef CATZ_MEM_LAYOUT_H')
    hdr.append('#define CATZ_MEM_LAYOUT_H')
    hdr.append('#include <stdint.h>')
    hdr.append(f'#define CATZ_IMAGE_SIZE {image_size}u')
    hdr.append(f'#define CATZ_GUARD_BASE {guard_base}u')
    hdr.append(f'#define CATZ_NUM_SEG {nseg}')
    hdr.append(f'#define CATZ_ENTRY_SEG {ne.cs}')
    hdr.append(f'#define CATZ_ENTRY_IP 0x{ne.ip:04X}u')
    hdr.append(f'#define CATZ_STACK_SEG {ne.ss}')
    hdr.append(f'#define CATZ_STACK_SP 0x{ne.sp:04X}u')
    hdr.append(f'#define CATZ_AUTO_DATA_SEG {ne.auto_data_seg}')
    ...
    if row:
        hdr.append('    ' + ','.join(row) + ',')
    hdr.append('};')
    hdr.append('#endif /* CATZ_MEM_LAYOUT_H */')
